...
test_server_session::test_server_session(void) : login_stat(LOGIN_STATE_NONE), last_send_command_time(0)
{
	username[0] = 0;
}

test_server_session::~test_server_session(void)
{
}
...
void test_server_session::on_closed( int reason )
{
...
void test_server_session::set_username( const char* username, int length /*= 0*/ )
{
	if (length == 0)
...
void test_server_session::send_command( const char* cmd, const char* data, int data_length /*= 0*/ )
{
