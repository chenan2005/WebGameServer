...
{
	DEC_REG_PROTO_MSG_HANDLE(test_s_pb_conf_session_manager)

public:

	test_s_pb_conf_session_manager(void);

	virtual ~test_s_pb_conf_session_manager(void);
...
protected:
	...
	std::map< std::string, test_s_pb_conf_session* > sessions;
};
