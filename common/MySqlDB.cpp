...
using namespace IODDB;

MYSQL* IODDB::common_mysql_connection = NULL;
...
bool IODDB::InitCommonMysqlConnection(const char* address, unsigned short port, const char* user, const char* pass, const char* dbname)
{
	if (IODDB::common_mysql_connection != 0)
		return false;
...
const char* IODDB::GetValueFromSzFieldIntoRecord( void* pRecord, const STOutFieldInfo& field, const char* szValue, unsigned long ulLength )
{
	...
	case IOD_FIELD_TYPE_TIME:
		{
			...
			else
			{
				...
				{
					*(time_t*)((char*)(pRecord) + field.m_nOffset) = iod_utility::MKTime(&tm1);
				}				
			}
...
const char* IODDB::FreeQuery( MYSQL* pConn, const char* pszSql, const char* pszRetValueFormat, ... )
{
...
const char* IODDB::FreeQuery( MYSQL* pConn, const char* pszSql, char* szRet /* = NULL*/ )
{
	{
...
void IODDB::TestFreeQuery( MYSQL* pConn )
{
	...
	const char* pszErr1 = IODDB::FreeQuery(pConn, "select RoleId as outId, RoleName as outName from TRoleInfo limit 1, 10", 
	...
	const char* pszErr2 = IODDB::FreeQuery(pConn, "select RoleId as outId, RoleName as outName from TRoleInfo limit 1", szQueryResult);

	uint64 nRetId;
	char szRetName[64];
	const char* pszErr3 = IODDB::FreeQuery(pConn, "select RoleId as outId, RoleName as outName from TRoleInfo limit 1", "%llu,%s", &nRetId, szRetName);
}
