...
struct listener_info* start_listener(IODSessionManager* session_creator, /*对话创建器 */ const char* bind_addr, /*监听地址，格式为 hostname:port */ int conn_timeout_secs /*= 120*/, /*每个连接的超时时间：超过该时间没有收到数据则关闭连接 */ int conn_highmark /*= DEFAULT_CONNECTION_HIGHMARK /*缓冲区上限：如果缓冲区中超过highmark字节未处理，则不再接收数据。0为不限制 */)
{
	if (!is_initialized)
		initialize();
...
struct connection_info* IODNetwork::start_connection(IODSession* session, /*处理消息的对话实例 */ const char* target_addr, /*目标地址，格式为 hostname:port */ int conn_timeout_secs /*= 120*/, /*每个连接的超时时间：超过该时间没有收到数据则关闭连接 */ int conn_highmark /*= DEFAULT_CONNECTION_HIGHMARK /*缓冲区上限：如果缓冲区中超过highmark字节未处理，则不再接收数据。0为不限制 */)
{
	if (!is_initialized)
		initialize();
	...
	bind_session_connection(conn_info, session, IODSession::SNS_CONNECTING);
...
void conn_eventcb(struct bufferevent *bev, short events, void *user_data)
{
	...
	if (events & BEV_EVENT_EOF) {
		...
		if (conn_info) {
			...
			if (conn_info->session) {
				bind_session_connection(conn_info, conn_info->session, IODSession::SNS_CONNECTED);
...
int process_none_session_data(IODSessionManager* session_creator, connection_info* conn_info)
{
	...
	while (1) 
	{
		...
		if (read_result > 0) {
			IODSession* session = session_creator->on_none_session_packet(conn_info, packet);
			if (session) {
				bind_session_connection(conn_info, session, IODSession::SNS_CONNECTED);
				session->on_packet(packet);
				break;
			}
			session_creator->netstatistics.recv_byte_count += read_result;
